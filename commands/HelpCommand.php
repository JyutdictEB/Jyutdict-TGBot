...
class HelpCommand extends UserCommand
{
     ...
     * @var string
     */
    protected $description = '顯示鋍仔嘅指令指引 Show bot commands help';
    ...
    public function execute(): ServerResponse
    {
        ...
        if ($command_str === '') {
            $text = '*Commands List* *指令清單* :' . PHP_EOL;
            ...
            if ($safe_to_show && count($admin_commands) > 0) {
                $text .= PHP_EOL . '*Admin Commands List* *Admin指令清單* :' . PHP_EOL;
                foreach ($admin_commands as $admin_command) {
                    $text .= '/' . $admin_command->getName() . ' - ' . $admin_command->getDescription() . PHP_EOL;
                }
            }

            $text .= PHP_EOL."欲知詳細指令用法 打 /help <指令>" . PHP_EOL . 'For exact command help type: /help <command>';
        ...
        if (isset($all_commands[$command_str]) && ($safe_to_show || !$all_commands[$command_str]->isAdminCommand())) {
            ...
            return $this->replyToChat(sprintf(
                '指令 Command: %s '. PHP_EOL .'(v%s)' . PHP_EOL .
                '功能 Description:'.PHP_EOL.' %s' . PHP_EOL .
                '用法 Usage:'.PHP_EOL. '%s',
                $command->getName(),
                $command->getVersion(),
        ...
        return $this->replyToChat('關於指令 `/' . $command_str . '` 冇見有' . PHP_EOL .'No help available: Command `/' . $command_str . '` not found', ['parse_mode' => 'markdown']);
    }
